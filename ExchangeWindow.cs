...
namespace NebliDex_Linux
{
    public partial class ExchangeWindow : Gtk.Window
    {
                ...
                base(Gtk.WindowType.Toplevel)
        {
            ...
            //Switch the Menu bar to the App
        ...
        public void AdjustCandlePositions()
        {
            ...
            if (change == 0)
            {
                Market_Percent.Markup = "<span font='16'><b>00.00%</b></span>";
            ...
            else if (change > 0)
            {
                ...
                if (change > 10000)
                {
                    Market_Percent.Markup = "<span font='16'><b>> +10000%</b></span>";
                }
                else
                {
                    Market_Percent.Markup = "<span font='16'><b>+" + change.ToString(CultureInfo.InvariantCulture) + "%</b></span>";
                }
            }
            else if (change < 0)
            {
            ...
            Chart_Last_Price.Markup = "<span font='13'><b>Last Price: " + String.Format(CultureInfo.InvariantCulture, "{0:0.########}", App.VisibleCandles[App.VisibleCandles.Count - 1].close) + "</b></span>";
            Chart_Last_Price.Yalign = 0.5f;
        ...
        private void Chart_MouseMove(object sender, MotionNotifyEventArgs e)
        {
            ...
            Chart_Mouse_Price.Markup = "<span font='13'>" + App.UTC2DateTime(gridtime).ToString("yyyy-MM-dd HH:mm") + " | " + String.Format(CultureInfo.InvariantCulture, "{0:0.########}", price) + "</span>";
            Chart_Mouse_Price.Yalign = 0.5f;
        }
        ...
        public void UpdateBlockrates()
        {
            ...
            if (dex_connected[0] == true && dex_connected[1] == true && dex_connected[2] == true && App.ntp1downcounter < 2)
            {
                ...
                Fee_Status.Markup = "<span font='12'><b>Current Blockchain Fees:</b></span>";
                if (App.using_blockhelper == true)
                {
                    Fee_Status.Markup = "<span font='12'><b>BlockHelper Active | Current Blockchain Fees:</b></span>";
                }
                ...
                CN_Fee.Markup = "<span font='12'>CN Fee: " + App.ndex_fee + "</span>";
                NEBL_Fee.Markup = "<span font='12'> | NEBL Fee: " + String.Format(CultureInfo.InvariantCulture, "{0:0.########}", Math.Round(App.blockchain_fee[0], 8)) + "/kb</span>";

                //Update Status Bar Fees
                if (App.MarketList[App.exchange_market].base_wallet != 0)
                {
                    Base_Pair_Fee.Markup = "<span font='12'> | " + App.MarketList[App.exchange_market].base_symbol + " Fee: " + String.Format(CultureInfo.InvariantCulture, "{0:0.########}", Math.Round(App.blockchain_fee[App.MarketList[App.exchange_market].base_wallet], 8)) + "/kb</span>";
                }
                else
                {
                ...
                if (App.critical_node == true)
                {
                    ...
                    CN_Info.Markup = "<span font='12'> | Tx Validating: " + App.cn_num_validating_tx + " (CNs Online: " + cn_online + ", " + percent + "% Chance of Validating)</span>";
                }
                else
                {
                    CN_Info.Text = "";
            ...
            {
                Fee_Status.Markup = "<span font='12'><b>Not Fully Connected:</b></span>";
        ...
        public void UpdateCandles()
        {
            ...
            Market_Percent.Markup = "<span font='16'><b>00.00%</b></span>";
            Chart_Last_Price.Markup = "<span font='13'><b>Last Price:</b></span>";
        ...
        public void SetupButtons()
        {
            ...
            Wallet_Box.ModifyBg(Gtk.StateType.Normal, new Gdk.Color(246, 246, 246));
            Trade_Information_Tabs.ModifyBg(Gtk.StateType.Normal, new Gdk.Color(246, 246, 246));
            ...
            Market_Percent.ModifyFg(Gtk.StateType.Normal, gdk_white);
            Market_Percent.Yalign = 0.5f;

            Gtk.Label buy_button_label = (Gtk.Label)Buy_Button.Children[0];
            buy_button_label.Markup = "<span font='16'>Buy " + App.MarketList[App.exchange_market].trade_symbol + "</span>";
            buy_button_label.Yalign = 0.5f;
            Gtk.Label sell_button_label = (Gtk.Label)Sell_Button.Children[0];
            sell_button_label.Markup = "<span font='16'>Sell " + App.MarketList[App.exchange_market].trade_symbol + "</span>";
            sell_button_label.Yalign = 0.5f;

            Gtk.Label withdraw_button_label = (Gtk.Label)Withdraw_Button.Children[0];
            withdraw_button_label.Markup = "<span font='13'>Withdraw</span>";
            withdraw_button_label.Yalign = 0.5f;
            Gtk.Label deposit_button_label = (Gtk.Label)Deposit_Button.Children[0];
            deposit_button_label.Markup = "<span font='13'>Deposit</span>";
            deposit_button_label.Yalign = 0.5f;
            ...
            chart_time_label.Markup = "<span font='13'>24 Hour</span>";
            ...
            //Setup the trayicon
            trayicon = new StatusIcon(App.App_Physical_Path + "/logo.ico");
            trayicon.Visible = false; //Hide it for now
            //trayicon.Activate += delegate { System.Diagnostics.Debug.WriteLine("Hello Man"); };
            trayicon.PopupMenu += OnTrayIconPopup;
            RunInBackgroundAction.Activated += delegate { this.Hide(); trayicon.Visible = true; };
            trayicon.Tooltip = "NebliDex will continue to run in the background to keep your open orders from closing.";

            //Hide the CN Fees Tab and Menu Options unless CN
            ToggleCNInfo(false);
        ...
        {
            if(statusbar_menu == null){
                statusbar_menu = new Menu();
                MenuItem re_open_app = new MenuItem("Show NebliDex Client");
                statusbar_menu.Add(re_open_app);
                re_open_app.Activated += delegate { this.Show(); trayicon.Visible = false; };
            }
            statusbar_menu.ShowAll();
            statusbar_menu.Popup();
        }
        ...
        private void Change_Chart_Timeline(object sender, EventArgs e)
        {
            ...
            { //Do not touch the visible candles unless we know no one else is
                ...
                {
                    //Change to 7 Day
                    chart_time_label.Markup = "<span font='13'>7 Day</span>";
                    chart_timeline = 1;
                }
                else
                {
        ...
        private async void Change_Market(object sender, EventArgs args)
        {
            ...
            if (which_market > -1)
            {
                ...
                if (App.critical_node == false)
                {
                    Market_Percent.Markup = "<span font='16'><b>LOADING...</b></span>"; //Put a loading status
                    Market_Percent.ModifyFg(Gtk.StateType.Normal, gdk_white);
                    if (current_ui_look == 1)
                    {
                        Market_Percent.ModifyFg(Gtk.StateType.Normal, blackc);
        ...
        private void Save_Backup_Dialog(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                dia.SetCurrentFolder(Environment.GetEnvironmentVariable("HOME"));
        ...
        private async void Open_Backup_Dialog(object sender, EventArgs e)
        {
            ...
            if (response == ResponseType.Ok)
            {
                ...
                try
                {
                    ...
                    await Task.Run(() => App.CheckWallet(this)); //Ran inline
