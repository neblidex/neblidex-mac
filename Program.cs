//This is the Mac Port of NebliDex Linux
...
namespace NebliDex_Linux
{
    public partial class App
    {
        ...
        {
            //Set the app_path to the environment variable
            App_Path = App_Physical_Path + "../../..";
            bool rungui = false;
            if (args.Length > 0)
            {
                ...
                {
                    rungui = true; //Default to show GUI
                }
            ...
            if(rungui == true){
                ...
                string theme_info = Environment.GetEnvironmentVariable("GTK_DATA_PREFIX");
                if (theme_info == null)
                {
                    Environment.SetEnvironmentVariable("GTK_DATA_PREFIX", App_Physical_Path + "/looks/");
                }else if(theme_info.Length == 0){
                    Environment.SetEnvironmentVariable("GTK_DATA_PREFIX", App_Physical_Path + "/looks/");
                }
                //Clear any sort of environmental variable that was automatically set by MONO
                Environment.SetEnvironmentVariable("GTK2_RC_FILES", " ");
                Application.Init();
                Gtk.Settings.Default.ThemeName = "Default"; //Set the theme
        ...
        public static bool CreateDatabase()
        {
            ...
            //Check if NebliDex Data folder exists
            ...
            if (Directory.Exists(App_Path + "/data") == false)
            {
                ...
                {
                    //Failed to Create the NebliDex data directory (possibly write restricted
                    if (run_headless == false)
                    {
                        Application.Invoke(delegate
                        {
                            MessageBox(null, "Notice", "Unable to create the NebliDex data directory. This folder prevents writing to it. Please change your folder permissions.", "OK");
                            Application.Quit();
                        });
                        return false;
                    }
                    ...
                    {
                        Console.WriteLine("Unable to create the NebliDex data directory. This folder prevents writing to it. Please change your folder permissions.");
                        Headless_Application_Close();
                    }
                }
            }
        ...
        public static bool VerifyTradeRequest(JObject tjs)
        {
            ...
            if (ntp1_wallet == false)
            {
                ...
                {
                    //This prevents maker from sending very small ETH or ERC20 amounts
